...
class RenderPipeline:
    ...
    def render(
        ...
        report("mixing audio", 65)
        audio = AudioMixer().mix(timeline, output_dir / "audio" / "mix.wav")
        report("writing captions", 75)
        SubtitleWriter().write(timeline, output_dir / "subtitles" / "captions.srt")
        report("compositing video", 85)
        video = VideoCompositor().compose(
            output_dir / "frames" / "frame-%06d.svg",
            project.render.fps,
            audio,
            output_dir / "final" / f"{project.name}.mp4",
        )
        self._metadata(
            output_dir, project.name, "complete", {"frames": len(frames), "video": str(video)}
        )
        report("complete", 100)
    ...
    def _metadata(output_dir: Path, project: str, status: str, details: dict) -> None:
        (output_dir / "metadata.json").write_text(
            json.dumps({"project": project, "status": status, **details}, indent=2),
            encoding="utf-8",
        )
